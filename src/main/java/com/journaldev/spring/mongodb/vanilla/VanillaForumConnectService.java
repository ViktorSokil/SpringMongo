  ...
  * @version 1.0 This object contains the client code for Vanilla VanillaForumConnectService
  * single-sign-on.
  */
 public class VanillaForumConnectService {

     private static final String SECRET = "96a02b827ecd536db825e07b5c0e7d95";
     private static final String CLIENT_ID = "1655329810";
    ...
    /**
     * Returns a JSONP formatted string suitable to be consumed by VanillaForumConnectService.
     * This is usually the only method you need to call in order to implement
     * VanillaForumConnectService.
     *
     ...
     * on Vanilla's VanillaForumConnectService configuration page.
     * @param secret: The secret for your site. This is usually configured on
     * Vanilla's VanillaForumConnectService configuration page.
     ...
     */
    public static String GetJsConnectString(Map user, Map request, Boolean secure) {
       Map error = null;
       ...
       long currentTimestamp = VanillaForumConnectService.Timestamp();

       if (secure) {
           String clientId = Val(request, "client_id");
          if (clientId == null) {
             error = VanillaForumConnectService.Error("invalid_request", "The client_id parameter is missing.");
          } else if (!clientId.equals(CLIENT_ID)) {
             error = VanillaForumConnectService.Error("invalid_client", "Unknown client " + Val(request, "client_id") + ".");
             ...
             error = VanillaForumConnectService.Error("invalid_request", "The timestamp is missing or invalid.");
          } else if (Val(request, "signature") == null) {
             error = VanillaForumConnectService.Error("invalid_request", "The signature is missing.");
          } else if (Math.abs(currentTimestamp - timestamp) > 30 * 60) {
             error = VanillaForumConnectService.Error("invalid_request", "The timestamp is invalid.");
          } else {
             // Make sure the timestamp's signature checks out.
             String timestampSig = VanillaForumConnectService.MD5(Long.toString(timestamp) + SECRET);
             if (!timestampSig.equals(Val(request, "signature"))) {
                error = VanillaForumConnectService.Error("access_denied", "Signature invalid.");
             }
          }
       }
       ...
       if (error != null) {
          ...
          SignJsConnect(result, CLIENT_ID, SECRET, true);
       } else {
       ...
       String json = VanillaForumConnectService.JsonEncode(result);
    ...
    /**
     * Sign a VanillaForumConnectService response. Responses are signed so that the site
    ...
    public static String SignJsConnect(Map data, String clientID, String secret, Boolean setData) {
       ...
       String signature = VanillaForumConnectService.MD5(sigStr.toString() + secret);
     ...
     * @return SSO string.
     */
    public static String SSOString(Map user) throws InvalidKeyException {
       ...
       try {
          ...
          Logger.getLogger(VanillaForumConnectService.class.getName()).log(Level.SEVERE, null, ex);
       }
